...
@CrossOrigin("*")
 public class DesignationController {
	...
	public ResponseEntity<ResponseDto> saveDesignation(@RequestBody Designation designation) {
		 ...
		 Designation desig = desigserv.saveDesignation(designation);
		 return ResponseEntity.status(HttpStatus.CREATED).body(new ResponseDto(HttpStatus.CREATED.toString(), "Designation "+desig.getDesignation()+" is saved successfully"));
	}
	...
	public ResponseEntity<ResponseDto> updateDesignation(@RequestBody Designation designation) {
		 ...
		 if(result > 0) {
			 return ResponseEntity.status(HttpStatus.CREATED).body(new ResponseDto(HttpStatus.CREATED.toString(), "Designation "+designation.getDesignation()+" is updated successfully"));
	     }
		 else {
		 	return ResponseEntity.status(HttpStatus.NOT_MODIFIED).body(new ResponseDto(HttpStatus.NOT_MODIFIED.toString(), "Designation "+designation.getDesignation()+" is not updated "));
		 } 
	}
